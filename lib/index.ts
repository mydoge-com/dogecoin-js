...
export async function generateDerivedHDPubkey(masterPrivKey): Promise<string> {
  ...
  let pub = _malloc(35)
  let priv = intArrayFromString(masterPrivKey)

  _generateDerivedHDPubkey(priv, pub)
...
export async function verifyPrivPubKeypair(privKey, pubKey): Promise<boolean> {
  ...
  // let pub = intArrayFromString(pubKey)
  // let priv = intArrayFromString(privKey)
  // let result = _verifyPrivPubKeypair(priv, pub, false)
